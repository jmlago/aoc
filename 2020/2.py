...
def detect_char(line):
    return line.split(":")[0].split(" ")[1]

def detect_positions(line):
...
def count_chars(line,char):
    passw =  line.split(" ")[-1]
    count = len(passw.split(char))-1
    return count

def is_valid(count, min, max):
    return 1 if count <=max and count >= min else 0

def is_valid_new(line,char,pos1,pos2):
    passw =  line.split(" ")[-1]
    aux = 0
...
,"8-11 t: tttttttcttm"
    ...
    for el in psswd_db:
        char = detect_char(el)
        pos1,pos2 = detect_positions(el)
        if new_policy:
            valid_passwds += is_valid_new(el,char,pos1,pos2)
        else:
            count = count_chars(el,char)
            valid_passwds += is_valid(count,pos1,pos2)
